...
# Initialize the FireCrawlWebSearchTool (web search tool)
    ...
    if st.session_state.messages[-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            with st.spinner("Fetching insights..."):
                ...
                # If the response is "Not Verified" or "Cannot substantiate", invoke the FireCrawlWebSearchTool for further verification
                if verification_status in ["Not Verified based on information in my Database", "Cannot substantiate this post at this time, check back later."]:
                    web_search_results = web_search_tool.search(query=user_input)
                    relevant_articles.extend(web_search_results)  # Add search results to the list of articles
